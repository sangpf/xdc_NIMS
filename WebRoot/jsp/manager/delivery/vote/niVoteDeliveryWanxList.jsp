      ...
      <section class="content">
        <div class="panel panel-default">
          <div class="panel-body">
            <form class="form-horizontal" action="${ctx}/platform/niVoteDeliveryWanxList.do"  method="post" id="formSearch">
              <div class="row mb10">
                <div class="col-sm-2">
          ...
          <div class="box">
            <div class="box-body">
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
                <c:forEach items="${niVoteDeliveryWanxs}" var="ns" varStatus="n">
	                <tr>
	                  <td><input type="checkbox"  value="${ns.deliveryid }" id="checkQuestionnaire" name="checkQuestionnaire">
	                  	  ...
	                  	  <input type="hidden" value="${ns.vqnid }" name="aqnid" id="aqnid">
	                  	  <input type="hidden" value="${ns.vStatus }" name="qStatus" id="qStatus">
	                  ...
	                  <td>
	                    <input type="button" onclick="editniVoteDeliveryWanx(${ns.deliveryid});" value="编辑">
					  </td>
	                </tr>
                </c:forEach>
                </tbody>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-footer">
					<button type="button" onclick="editniVoteDeliveryWanxSave()"
...
//弹出运营调整模态框
 function OperationChange(deliveryid,vqnId){
 	$.ajax({
 		url:'${ctx}/platform/niVoteDeliveryWanxOperateModal.do',
 		data:{
 		...
 		success:function(data){
		   	...
		   	if(data.success){
		   		$("#sqnName").val(data.niVoteDeliveryWanx.collectnum);  //计划采集分数
		   		$("#sqnTime").val(data.niVoteDeliveryWanx.btime);   //上线时间
		   		$("#trueOrderNum").val(data.trueOrderNum);    //实际收集分数
		   		$("#showNum").val(data.niVoteDeliveryWanx.collectednum);  //页面显示分数
		   		
		   		$("#delivery_id").val(deliveryid);
		   		$("#sqn_Id").val(vqnId);
		   	}
		//   	adjust_ShowNum = setInterval("adjustShowNum("+deliveryid+","+vqnId+")",1000*60*3); //定时检查开关状态,设置显示收集份数
		   	set_interval = setInterval("changeShowNum("+deliveryid+","+vqnId+")",1000*3);  //定时请求显示运营调整数据
 		}
 	});
 }
 //请求显示运营调整数据
 function changeShowNum(deliveryid,vqnId){
/*  	if(vqnId == "" || deliveryid == ""){
 	...
 	$.ajax({
 		url:'${ctx}/platform/findVoteShowOrderNum.do',
 		type:'post',
 		dataType:'json',
 		data:{
 			vqnId : vqnId,
 ...
 //定时根据开关修改显示收集份数
 /* function adjustShowNum(deliveryid,vqnId){
 	...
 	if(datau1.checked){
		...
		//将调整数据设置为显示收集份数
		$.ajax({
			url:'${ctx}/platform/AdjustVoteDeliveryWanxOperate.do',
			dataType:'json',
			data:{
				bj : bj,
				deliveryid : deliveryid,
...
function surveyDeliveryOperateSave(){
  ...
  function uploadpic(){
	  ...
	  $.ajax({
		  url: '${ctx}/platform/niVoteDeliveryWanxImgUpload.do?sqnId='+sqnId, 
		  type:'post',
		  data: formData,
		  dataType:'json',
          async: false,  
...
//保存编辑
  function editniVoteDeliveryWanxSave(){
	 ...
	 if(deliveryid_show == ""){
 	 ...
 	 $.ajax({
 		 url:'${ctx}/platform/niVoteDeliveryWanxEditorSave.do',
 		 dataType:'json',
 		 type:'post',
 		 data:{
  ...
  //编辑页面跳转
  function editniVoteDeliveryWanx(deliveryid){
	  window.location.href = "${ctx}/platform/niVoteDeliveryWanxSaveMESJump.do?deliveryid="+deliveryid;
  }
  
  //编辑页面跳转
  function editniVoteDeliveryWanx_bak(deliveryid){
	  $.ajax({
		  url:'${ctx}/platform/niVoteDeliveryWanxEditor.do',
		  type:'post',
		  data:{
			  deliveryid : deliveryid
		  },
		  ...
		  success:function(data){
			  ...
			  $("#sqnname_show").val(data.nvq.vqnname);  //问卷名称
			  $("#editor_show").val(data.nvq.editor);   //出题人
	...
	function addQuestionnaire(){
		window.location.href = "${ctx}/platform/niVoteDeliveryWanxAdd.do";
	}
	...
	function putQuestionnaire(){
		...
		//进行投放
		$.ajax({
			url:'${ctx}/platform/niVoteDeliveryWanxPut.do',
			type:'post',
			dataType:'json',
			data:{
			...
			success:function(data){
				showMessageDialog(data.msg, function(){
					if(data.success){
						window.location.href = "${ctx}/platform/niVoteDeliveryWanxList.do";
					}
				});
			}
		});
	...
	function withdrawnQuestionnaire(){
		...
		showConfirmDialog("确定要撤回问卷吗?", function(check){
			if(check){
				$.ajax({
					url:'${ctx}/platform/niVoteDeliveryWanxWithdrawn.do',
					type:'post',
					dataType:'json',
					data : {
						deliveryId_list : deliveryId_list
	...
	function stopQuestionnaire(){
		...
		$.ajax({
			url:'${ctx}/platform/niVoteDeliveryWanxStop.do',
			type:'post',
			dataType:'json',
			data : {
				deliveryId_list : deliveryId_list
	...
	function regenerationQuestionnaire(){
		...
		$.ajax({
			url:'${ctx}/platform/niVoteDeliveryWanxRegeneration.do',
			type:'post',
			dataType:'json',
			data:{
				deliveryId_list : deliveryId_list
	...
	function overQuestionnaire(){
		...
		showConfirmDialog("确定要终止问卷吗?", function(check){
			if(check){
				$.ajax({
					url:'${ctx}/platform/niVoteDeliveryWanxOver.do',
					type:'post',
					dataType:'json',
					data:{
						deliveryId_list : deliveryId_list
